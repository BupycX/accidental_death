//Сделать рандом
//присвоить к переменной основного массива из которого берется случайная переменная в которой записан текст
//вывести текст из полученной случайной переменной присвоенной в массиве
...
function new_game()
	{
		...
		var ansver_jail = [
		{memory:"Попытаться вспомнить"},
		{call_guardian:"Позвать стражника"},
		{jailbreak: "Попытаться сбежать"}
		]; 
		//  alert( arr[0].name ) варианты ответов в тюрьме [память, зов стража, побег из тюрьмы]
		
		var ansver_obshiu = ['look_around']; //общие варианты [Осмотреться]
		
		//примерная работа вывода описания из массива
		// var ansver_jail = [{memory:'Описание'},'call_guardian','jailbreak'];
		// alert(ansver_jail[0].memory);
...
// var name = "Миша, Даша, Паша"; //переводит переменные в массив
// var arr = name.split(', ')